...
Each fixture captures a failure I kept checking by hand before extracting the
rule into dibble. The commands are copy-paste-able from the repository root and
take a few seconds. `tailwind-v4-tokens` is the exception because it is a
knowledge skill with no checker script.
